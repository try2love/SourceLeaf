...
- [x] 建立大文档选区延迟与插入光标闪烁的可重复性能回归
- [x] 优化源码覆盖层重绘，恢复系统级即时选区反馈和动态光标
- [x] 持久恢复最后成功 PDF，关闭并重开应用后无需重新编译
- [x] 让对话输入区可纵向调整，并加入快捷 Prompt 工具栏
- [x] 重做长提示词设置页，补齐内置提示词英文正文
- [x] 增加 Provider 健康检查、连接状态、模型与思考深度控件
- [x] 调研并以安全的可配置 CLI Profile 兼容 WorkBuddy/CodeBuddy
- [x] 完成双语、本地化、性能/行为/视觉回归、Release 安装和 GitHub 推送
- **状态：** complete

### 阶段 12：连续预览、全局交互与专业编辑能力
- [x] 建立 PDF/图片连续滚动、缩放、工具栏尺寸与面板边界回归
- [x] 实现 PDF 纵向连续浏览、缩略图、缩放、导出与自动编译控制
- [x] 实现图片预览缩放，并统一 PDF/图片/源码/对话面板标签高度
- [x] 增加应用级界面字体缩放，修复提示词滚动、标签和文件行命中区
- [x] 增加查找替换、表格/图片等 LaTeX 插入工具和常用快捷键
- [x] 让加粗等包裹命令可切换，并让撤销恢复原始选区
- [x] 让拖动选区在鼠标移动时即时显示且保持大文档性能预算
- [x] 完成中英本地化、行为/视觉/性能回归、Release 安装及 GitHub 推送
- **状态：** complete
...
| 阶段 12 首轮专业编辑测试找不到 table/figure 命令 | 1 | 符合预期红灯；实现表格/图片模板、切换格式及对应 UI 后转绿 |
