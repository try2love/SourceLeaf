...
### 阶段 13：编辑可靠性、原生交互与完整会话系统
- [x] 复现并锁定乱序输入、光标回跳、拖选滞后、接受后滚顶和查找主题异常
- [x] 重构编辑器单一事实源、选区/滚动保持、原生查找替换与常驻 AI 目标按钮
- [x] 实现真正的应用级字体缩放、全按钮状态提示、文件夹整行展开和宽命中分隔条
- [x] 修复图片居中/平移、项目 PDF 多页预览及 PDF Control+滚轮缩放
- [x] 完善保存策略、未保存关闭确认及手动保存与自动编译约束
- [x] 重构普通聊天/编辑目标/只读上下文语义，加入取消、可见处理状态和发送键偏好
- [x] 实现消息选择/复制/编辑/重答/时间、新建/重命名/切换会话和系统提示词
- [x] 持久保留已接受 Diff，消除重复菜单，并升级历史为可审阅 Git 风格对比与明确恢复流程
- [x] 完成中英本地化、性能/行为/视觉回归与 Release 安装
- [ ] 完成 GitHub 提交、标签和推送
- **状态：** in_progress
